...
def load_css() -> None:
    # The normal repository layout keeps the stylesheet at
    # <repository root>/assets/style.css.  Also accept 311/assets/style.css as
    # a friendly fallback for repositories assembled through GitHub's web UI.
    css_candidates = (
    ...
    if css_path is None:
        logging.warning("Stylesheet not found: assets/style.css")
        st.warning(
            "디자인 파일(assets/style.css)을 찾지 못했어요. "
            "기능은 계속 사용할 수 있지만 GitHub의 파일 위치를 확인해 주세요."
        )
        return
    ...
    try:
        ...
        logging.exception("Unable to read stylesheet: %s", css_path)
        st.warning("디자인 파일을 읽지 못했어요. 잠시 뒤 다시 시도해 주세요.")
        return
